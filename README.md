...
DevX Mux reserves the canonical names in the table plus the obsolete `devx-mux`, `pr-title-description`, and `staged-pr-review` names in the skill directories it manages. The installer builds and validates the complete canonical-link and legacy-cleanup mutation plan before writing anything, rejects any mutation path that overlaps the packaged source tree, then applies the plan. This makes setup deterministic after reinstalling or moving the package without risking its source checkout or trying to infer ownership from stale link targets. The canonical orchestration workflow and shared browser transport live in `mux-orchestrate`.
