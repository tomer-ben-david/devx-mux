...
test("pull request review reads stated intent and discloses unavailable discussion", () => {
  const prompt = buildReviewPrompt({
  ...
  assert.match(prompt, /state exactly what context could not be read in the final report/);
});
